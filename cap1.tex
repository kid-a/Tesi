...
\end{itemize}
%
%% --> La scelta del sistema di acquisizione e` un compromesso tra necessita` di dati real time
%%     e complessita` di installazione del sistema

%% problemi legati ai sensori
...
%% Al fine di produrre le informazioni rilevanti per ciascuna delle classi di utenti sopra
%% indicate, \`e necessario monitorare un insieme di \emph{grandezze di campo} mediante 
%% l'uso di 


%% %% Produrre le informazioni rilevanti per ciascuna delle classi di utenti sopra indicate
%% %% richiede, come prima cosa, l'identificazione delle grandezze di campo da monitorare.
%% %% %
%% %% Ad esempio, produrre le informazioni relative alla produzione energetica per i 
%% %% proprietari degli impianti, richiede la conoscenz


%% Estrapolare e/o produrre le informazioni richieste da ciascuna delle classi di utenti
%% sopra elencate, richiede \Item{i} l'identificazione delle grandezze 

%% -> quali variabili misurare?
%% -> ogni quanto misurarle?

%% quale grandezza misurare? quante misurarne?
%% la semplice potenza in uscita non basta !

%% The objective of the monitoring system is to comprehensively
%% collect and integrate all relevant data. This includes voltage,
%% current and energy measurements of the nett photovoltaic
%% system, its ambient operating environment and local
%% atmospheric conditions. This data can then be used for short
%% term purposes such as monitoring the system’s operating
%% status as well as for multi-year objectives such as monitoring
%% the deterioration of components with time, estimation of the
%% overall efficiency and energy generation and so on. The work
%% was also driven by the need to generate data on BIPV systems
%% in northern latitude with extreme temperature variations. It
%% was also hoped that the data would help in the development of
%% more accurate modeling of such systems.
...
%% terminologia dal \emph{controllo dei processi industriali}: 
%% la produzione di energia elettrica mediante fotovoltaico pu\`o essere
%% modellata, infatti, come un \emph{processo continuo} caratterizzato dalle 
%% seguenti \emph{variabili di stato}:
%% %
%% \begin{itemize}
%% \item la corrente che attraversa ogni singola stringa
%% \item la potenza in ingresso ad ogni inverter
%% \item la potenza in uscita ad ogni inverter
%% \item la potenze attive e reattive scambiate con la rete di distribuzione
%% \end{itemize}
%% %
...
%% \begin{equation}
%%   N_{sG} V_{mMr} N_{pG} I_{mMr} (PSH) = E_{DC}
%% \end{equation}

%% V_{mMr} e I_{mMr} sono tensione e corrente al MPP (standard test conditions)
%% N_{sG} numero di moduli in serie
%% N_{pG} numero di serie
